...
type Service struct {
	...
	lastCSActivity atomic.Int64 // UnixNano of last confirmed CS event (any SMS received)
...
// voice call to the scooter's own number.
//
// Why we call ourselves: O2/Lebara DE's MSC runs a ~15-minute SGs implicit
// detach timer (3GPP TS 29.118 §5.3.3) against each EPS-attached UE. When it
// expires the MSC silently drops the SGs record, and SMS delivery fails because
// the MSC can no longer page the modem. Any CS NAS uplink message resets the
// timer. An MO voice call emits an Extended Service Request (MO_CS_FB) from
// the modem to the MME via LTE NAS; the MME then sends SGsAP-SERVICE-REQUEST
// to the MSC, resetting the timer. The modem does CSFB to EDGE for ~2 s and
// returns to LTE — internet stays connected throughout, IP unchanged. We dial
// our own number so the call never connects (busy-to-self), meaning it is
// completely free and leaves no call record.
...
//
// Fallback — refreshSGsViaCFUN4: if voice keepalive fails (SGs already expired),
